...
**日界与标题日期（2026-09-22 新增，均在 `reflect-plan.ts`）**：

| 纯函数 | 语义 |
|---|---|
| `pickSnapshot(now)` | 取「**最近一个已完结日**」的 6 维快照——**不是** `today` 的当日计数器（00:00 刚翻页时当日计数器为零值，用它会把「昨天的一整天」写成 0） |
| `toTitleDate(d)` | 标题里的日期，**必须与数据同源**（同取 `pickSnapshot().date`）——此前标题日期在**模块级常量**里求值 ⇒ 冻结在 web 启动日 |
| `buildReflectionPrompt(now, emotion)` | 组装反思提示文本：数据与标题日期**一次算出**，杜绝两处各取一次「现在」 |
...
| A9 | 6 维读数取「**最近已完结日**」的快照，不是当日计数器（00:00 刚翻页不出现零值） | `tests/reflect-plan.test.mjs`（`pickSnapshot` 用例）；线上三阶段真实报文：`09-21 / 09-22 零值` → `09-22 / 09-21` → **`09-21 / 09-21` ✓** | ✓ 已实测（2026-09-22） |
| A10 | 标题日期与数据**同源**（不冻结在模块加载时刻） | `tests/reflect-plan.test.mjs`（`toTitleDate` 用例）；修复后线上标题日期 == 快照日期 | ✓ 已实测（2026-09-22） |
...
**2026-09-22 日界错位修复（t-d9497d70 · 两处独立缺陷叠加）**

- 语义**被修正（两处，必须同修）**：① `buildEmotionSummary` 取的是 `st.today` 的**当日计数器**——00:00 刚翻页时它是零值 ⇒ 反思块里 6 维读数全为 0；② `REFLECTION_PROMPT` 是**模块级常量**，其中的 `new Date()` 在**模块加载时**求值 ⇒ 标题日期冻结在 web 启动那一天。
- **只修一处的中间态实测（关键证据）**：标题 `09-22` / 数据 `09-21` ⇒ 仍不一致，证明这两处是**独立缺陷**，修一处不够。
- 修法：抽出 `pickSnapshot`（取最近**已完结**日）+ `toTitleDate` + `buildReflectionPrompt`（标题与数据**同取** `pickSnapshot().date`），删掉模块级常量 `REFLECTION_PROMPT`；`emotionSummary()` 改为返回日期。判据：真实报文三阶段对比 `09-21 / 09-22 零值` → `09-22 / 09-21` → **`09-21 / 09-21` ✓**；测试 30 → 34 项全绿。commit `265813a` + `91f895a`。
- 教训：**「模块级常量 / 默认参数里的 `new Date()`」是隐藏的时间冻结点**——凡是输出里出现日期，都要问一句：这个日期是**取数时刻**算的，还是**模块加载时刻**算的？（AGENTS.md §5.9 规则 6「读数必须自带范围标注」的时间维度实例。）
